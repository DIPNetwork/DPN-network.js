...
class Peer extends EventEmitter {
  ...
  _parsePacketContent (data) {
    switch (this._state) {
      ...
      case 'Body':
        ...
        console.log(`Peer receiving information： ${prefix} (message code: ${code} - ${obj.offset} = ${msgCode}) ${this._socket.remoteAddress}:${this._socket.remotePort}`)
        try {
  ...
  _sendAuth () {
    if (this._closed) return
    console.log(`Send auth (EIP8: ${this._EIP8}) to ${this._socket.remoteAddress}:${this._socket.remotePort}`)
    if (this._EIP8) {
  ...
  _sendAck () {
    if (this._closed) return
    console.log(`Send ack (EIP8: ${this._eciesSession._gotEIP8Auth}) to ${this._socket.remoteAddress}:${this._socket.remotePort}`)
    if (this._eciesSession._gotEIP8Auth) {
  ...
  _sendHello () {
    console.log(`Send HELLO to ${this._socket.remoteAddress}:${this._socket.remotePort}`)
    const payload = [
