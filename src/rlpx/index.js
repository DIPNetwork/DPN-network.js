...
class RLPx extends EventEmitter {
  ...
  async connect (peer) {
    ...
    if (this._peers.has(peerKey)) {
      throw new Error('Connection repeat')
    }
    if (this._getOpenSlots() === 0) {
      throw new Error('Connected nodes exceed Max')
    }
  ...
  _connectToPeer (peer) {
    ...
    console.log(`Connect to peer: address =${opts.address}`)
  ...
  _onConnect (socket, peerId) {
    console.log(`Wait for a handshake : ${socket.remoteAddress}:${socket.remotePort}`)
    const peer = new Peer({
      socket: socket,
      remoteId: peerId,
  ...
  _refillConnections () {
    ...
    console.log(`Refill Connections：queue size: ${this._peersQueue.length}, open slots: ${this._getOpenSlots()}`)
